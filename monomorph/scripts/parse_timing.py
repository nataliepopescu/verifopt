...
def parse(path):
        ...
        for line in f:
            ...
            if mode == "timing":
                ...
                if m:
                    ...
                    timing_rows.append(
                        (bb_n_from_label(label), label, cat,
    ...
    return timing_rows, scope_rows, selftime_rows
...
def main():
    ...
    timing_rows, scope_rows, selftime_rows = parse(args.logfile)

    write_csv(
        timing_rows,
        ["bb_visit", "label", "category", "total_ms", "count", "avg_ms", "min_ms", "max_ms"],
        f"{args.outdir}/timing_windows.csv",
    )
    write_csv(
        scope_rows,
        ["bb_visit", "label", "scope", "category", "total_ms", "count", "avg_ms"],
        f"{args.outdir}/timing_by_scope.csv",
    )
    write_csv(
        selftime_rows,
    ...
    print(f"parsed {len(timing_rows)} timing rows, {len(scope_rows)} scope rows, "
    ...
    #print("\n=== per-category avg_ms trend across windows ===")
