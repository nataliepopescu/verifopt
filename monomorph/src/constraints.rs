...
/// Depth at which a nested `Adt` constraint gets flagged as "deep" even if
/// it isn't (yet) confirmed to be the exact same `AdtDef` recurring - purely
/// diagnostic, chosen well below anything that could plausibly occur in a
/// legitimately-deep but finite real type (e.g. deeply nested tuples/structs
/// from ordinary code), so a hit here is a strong signal of runaway growth
/// rather than a false positive.
const RUNAWAY_ADT_DEPTH_WARN: usize = 15;

/// Diagnostic-only: walks a `Constraints` value's `Adt` fields looking for
/// the same `AdtDef` appearing more than once along a single recursive path
/// (i.e. a wrapper type nesting inside itself, e.g. `Range<Range<...>>`),
/// and separately flags any path whose depth crosses
/// `RUNAWAY_ADT_DEPTH_WARN` regardless of whether it's a literal repeat.
/// Added to confirm/measure the loop-carried type-nesting-explosion
/// hypothesis before committing to a fix; safe to remove once confirmed.
/// `label` is just free-form context (e.g. the merge call site's scope)
/// to make the resulting `warn!` lines greppable/attributable.
pub fn debug_check_runaway_adt_nesting(constraints: &Constraints, label: &str) {
    for constraint in constraints.inner.iter() {
        if let Some(cfc) = &constraint.cfc {
            let mut path: Vec<DefId> = Vec::new();
            check_adt_nesting_inner(cfc, &mut path, label);
        }
    }
}

fn check_adt_nesting_inner(cfc: &RunningConstraint, path: &mut Vec<DefId>, label: &str) {
    match cfc {
        RunningConstraint::Adt(adtdef, _genargs, _variant, fields) => {
            let this_defid = adtdef.0;
            let depth = path.len() + 1;
            if path.contains(&this_defid) {
                warn!(
                    "RUNAWAY ADT NESTING at {}: {:?} reappears inside itself at depth {} (path: {:?})",
                    label,
                    adtdef.0.name(),
                    depth,
                    path,
                );
            } else if depth >= RUNAWAY_ADT_DEPTH_WARN {
                warn!(
                    "DEEP ADT NESTING at {}: depth {} reached, innermost {:?} (path: {:?})",
                    label,
                    depth,
                    adtdef.0.name(),
                    path,
            ...
            path.push(this_defid);
            for field_constraints in fields.values() {
                for field_constraint in field_constraints.inner.iter() {
                    if let Some(field_cfc) = &field_constraint.cfc {
                        check_adt_nesting_inner(field_cfc, path, label);
                    }
                }
            }
            path.pop();
        }
        RunningConstraint::Ptr(inner) => {
            if let Some(inner_cfc) = &inner.cfc {
                check_adt_nesting_inner(inner_cfc, path, label);
            }
        }
        _ => {}
    }
