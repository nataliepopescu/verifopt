...
pub fn mk_vec() -> Vec<Box<dyn Animal>> {
	vec![]
}

pub fn run_best(xs: &[Box<dyn Animal>], cat: &Cat) -> usize {
	let mut ret = 0;
    for _ in xs {
...
pub fn run_not_rw(xs: &[Box<dyn Animal>]) -> usize {
	let mut ret = 0;
    for x in xs {
		ret = x.speak();
	}
...
pub fn run_src_rw(xs: &[Box<dyn Animal>], cat: Box<dyn Animal>) -> usize {
	let mut ret = 0;
    for x in xs.iter() {
...
pub fn run_src_rw_fallback(xs: &[Box<dyn Animal>], cat: Box<dyn Animal>, dog: Box<dyn Animal>) -> usize {
	let mut ret = 0;
    for x in xs.iter() {
		let x_vtable = core::ptr::metadata(&**x);
		let cat_vtable = core::ptr::metadata(&*cat);
